...
remote_mem_size_base=28 # 48 gb Remote memory size
...
#            	port=`echo $memory | cut -d ' ' -f2`
          ...
          ssh -o StrictHostKeyChecking=no $ip	"ulimit -c 50000000 && cd $BIN_HOME && numactl --physcpubind=31 ./memory_server_term  $port $(($remote_mem_size+10)) $((2*$i +1)) $remote_mem_size | tee -a $log_file.$ip " &
          sleep 1
          i=$((i+1))
#        	if [ "$i" = "$node" ]; then
#        		break
...
zipfian_alpha_range="0.1 0.99 10" #make sure workload = 1 if we want to test zipfian.
#
op_range="1" # use 1
#cache_th=0.5
for workload in $workload_range
