...
#    git clone https://github.com/google/cityhash &&
    ssh -o StrictHostKeyChecking=no ${compute_shard[n]}  " cd cityhash/ && ./configure && make all check CXXFLAGS='-g -O3' && sudo make install && cd .. && sudo apt-get update && sudo apt-get install -y libnuma-dev htop libmemcached-dev libboost-all-dev" &
...
#    screen -d -m pwd && cd /users/Ruihong && git clone --recurse-submodules $github_repo && cd MemoryEngine/ && mkdir release &&  cd release && cmake -DCMAKE_BUILD_TYPE=Release .. && sudo apt install numactl -y &&screen -d -m pwd && cd /users/Ruihong && git clone --recurse-submodules $github_repo && cd MemoryEngine/ && mkdir release &&  cd release && cmake -DCMAKE_BUILD_TYPE=Release .. && sudo apt install numactl -y &&
    ...
    ssh -o StrictHostKeyChecking=no ${memory_shard[n]}  " cd cityhash/ && ./configure && make all check CXXFLAGS='-g -O3' && sudo make install && cd .. && sudo apt-get update && sudo apt-get install -y libnuma-dev htop libmemcached-dev libboost-all-dev" &
#   git clone https://github.com/google/cityhash &&
