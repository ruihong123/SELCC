...
//  pattern[rdma_mg->getMyThreadID()][page_addr.nodeID]++;
    ...
    void
    Btr::global_lock_and_read_page(ibv_mr *page_buffer, GlobalAddress page_addr, int page_size, GlobalAddress lock_addr,
...
//        assert((page->local_lock_meta.current_ticket == page->local_lock_meta.issued_ticket && page->local_lock_meta.local_lock_byte == 0) || (page->local_lock_meta.current_ticket != page->local_lock_meta.issued_ticket && page->local_lock_meta.local_lock_byte == 1));
        ...
        printf("Allocate slot for page 1, the page global pointer is %p , local pointer is  %p, hash value is %lu level is %d, this thread is %d\n",
               page_addr, mr->addr, HashSlice(page_id), level, rdma_mg->thread_id);
